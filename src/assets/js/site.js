...
const I = {
  phone: '<svg class="icon" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.37 1.9.72 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.35 1.85.59 2.81.72A2 2 0 0 1 22 16.92z"/></svg>',
  ...
  arrow: '<svg class="icon" viewBox="0 0 24 24"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>',
  ...
  award: '<svg class="icon" viewBox="0 0 24 24"><circle cx="12" cy="8" r="7"/><path d="M8.21 13.89 7 23l5-3 5 3-1.21-9.12"/></svg>',
  ...
  checkCircle: '<svg class="icon" viewBox="0 0 24 24"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m22 4-10 10-3-3"/></svg>',
  ...
  square: '<svg class="icon" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>',
  ...
  chevL: '<svg class="icon" viewBox="0 0 24 24"><path d="m15 18-6-6 6-6"/></svg>',
  chevR: '<svg class="icon" viewBox="0 0 24 24"><path d="m9 18 6-6-6-6"/></svg>',
...
function renderHeader() {
  ...
  const navLinks = NAV.map(n =>
    `<a href="${n.href}"${n.href === here ? ' aria-current="page"' : ""}>${n.label}</a>`).join("");
...
const HOME_T = [
  { quote: "David and Tonya helped us buy sight-unseen while still in Germany. The process was seamless and felt like family.", name: "Lt. Col. Harris", role: "U.S. Army · PCS to Schofield", img: "assets/images/avatar-1.jpg" },
  { quote: "Sold our home in 4 days, above asking price, while we were already on the mainland. Unmatched professionalism.", name: "The Parkers", role: "Navy Family · Pearl Harbor", img: "assets/images/avatar-2.jpg" },
  { quote: "They treated us like ohana from day one. We couldn't have asked for a smoother PCS move to Oahu.", name: "Capt. Rodriguez", role: "USMC · MCBH Kaneohe Bay", img: "assets/images/avatar-3.jpg" },
  { quote: "Living on Oahu is expensive but they found us a gem within BAH. Forever grateful for their care.", name: "Sgt. J. Mitchell", role: "Air Force · Hickam Field", img: "assets/images/avatar-4.jpg" }
];
...
function initHomeSlider() {
  ...
  const img = el("[data-img]", root), q = el("[data-q]", root), n = el("[data-n]", root), r = el("[data-r]", root);
  const dotsHost = el("[data-dots]", root);
  dotsHost.innerHTML = HOME_T.map((_, k) => `<button data-dot="${k}" aria-label="Slide ${k + 1}"></button>`).join("");
  const render = () => {
    const t = HOME_T[i];
    img.src = t.img; img.alt = t.name; q.textContent = `"${t.quote}"`; n.textContent = t.name; r.textContent = t.role;
    els("[data-dot]", root).forEach((d, k) => d.classList.toggle("active", k === i));
  };
  const go = (next) => { i = (next + HOME_T.length) % HOME_T.length; render(); };
  el("[data-prev]", root).addEventListener("click", () => go(i - 1));
  el("[data-next]", root).addEventListener("click", () => go(i + 1));
  els("[data-dot]", root).forEach(d => d.addEventListener("click", () => go(+d.dataset.dot)));
...
function initAgentModals() {
  ...
  buttons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      el("[data-m-chips]", backdrop).innerHTML = a.specialties.map(s => `<span>${s}</span>`).join("");
